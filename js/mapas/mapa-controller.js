...
let ultimoPaisSelecionado = null; // <- NOVO: para evitar flicker ao tabular
...
document.getElementById('country').addEventListener('change', function() {
    ...
    if (paisId === ultimoPaisSelecionado) {
        // Se não mudou, não faz nada (evita flicker ao tab/tab+shift, clique, etc)
        return;
    }
    ultimoPaisSelecionado = paisId;
...
// Ao limpar o campo país, reseta tudo
document.getElementById('country').addEventListener('input', function() {
    const paisId = this.value.trim();
    if (!paisId) {
        ultimoPaisSelecionado = null; // <- Também limpa o último país
        carregarMapa('svg/europamain.svg');
        paisSelecionado = null;
        zonaSelecionada = null;
        document.getElementById('zone').value = '';
